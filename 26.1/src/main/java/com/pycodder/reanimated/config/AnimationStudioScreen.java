...
public class AnimationStudioScreen extends Screen {
    ...
    private void cycle(Supplier<Component> message, Runnable onClick) {
        Button button = Button.builder(message.get(), b -> {
            ...
            restartPreview();
        }).pos(listX, 0).size(listW, 20).build();
    ...
    private void renderGraph(GuiGraphicsExtractor context) {
        int pad = 6;
        int x0 = rightX + pad;
    ...
    private void renderPreviews(GuiGraphicsExtractor context, float delta) {
        int total = working.totalMs(PREVIEW_ELEMENTS);
        float loop = loopElapsed(total);
    ...
    private void renderPreviewPanel(GuiGraphicsExtractor context, float delta, String labelKey,
                                    int top, int bottom, float elapsedMs) {
        context.fill(rightX, top, rightX + rightW, bottom, 0xB0000000);
        context.text(font, Component.translatable(labelKey), rightX + 6, top + 3, 0xFFDDDDDD);
    ...
    private void drawUiElement(GuiGraphicsExtractor context, int x, int y, int w, int h, float alpha) {
        context.fill(x - 1, y - 1, x + w + 1, y + h + 1, argb(0x2B2B2B, alpha));
        context.fill(x, y, x + w, y + h, argb(0xC6C6C6, alpha));
        context.fill(x, y, x + w, y + 6, argb(0x8B8B8B, alpha));
        int sy = y + 9;
        for (int s = 0; s < 4; s++) {
            int sx = x + 4 + s * 11;
            if (sx + 8 > x + w || sy + 8 > y + h) break;
            context.fill(sx, sy, sx + 8, sy + 8, argb(0x373737, alpha));
        }
    }
    ...
    public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent event, boolean doubled) {
        if (event.x() >= rightX && event.x() <= rightX + rightW && event.y() >= openTop && event.y() <= closeBottom) {
            restartPreview();
            return true;
        }
