 ...
 * списка здесь ряд кнопок-вкладок сверху: за раз показывается одна секция, и
 * всё помещается даже на экране 320×240 без прокрутки. Прокрутку намеренно не
 * делаем — mouseScrolled/mouseClicked пришлось бы трогать, а их сигнатуры
 * менялись между версиями.
 *
...
public class ReAnimatedConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        addRenderableWidget(Button.builder(Component.translatable("reanimated.opt.tested_mods"),
                ...
                .bounds(this.width - 120, 4, 116, 16).build());

        // --- Кнопки-вкладки: два ряда по три ---
        ...
        switch (section) {
            ...
            default -> { // Общее
                rows.add(cycler("reanimated.opt.preset", () -> c.uiPreset, v -> c.uiPreset = v, UiPreset.values(), p -> p.display));
    ...
    private <T extends Enum<T>> Button cycler(String key, Supplier<T> get, Consumer<T> set,
        ...
        return Button.builder(enumLabel(key, display.apply(get.get())), b -> {
            T nx = values[(get.get().ordinal() + 1) % values.length];
            set.accept(nx);
            ReAnimatedConfig.get().save();
