 ...
 * примитивы (Button + свой слайдер), без OptionInstance/OptionsList и без
 * переопределения mouseClicked — поэтому один код работает на 1.21.1–1.21.11.
 * Ссылка "mod by @pycodder" — кнопка через ConfirmLinkScreen.confirmLinkNow.
 */
public class ReAnimatedConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            w.setWidth(colW);
            w.setPosition(startX + col * (colW + gap), startY + r * (rowH + vgap));
