...
public class SodiumIntegration implements ConfigEntryPoint {
    ...
    private static OptionPageBuilder general(ConfigBuilder b, ReAnimatedConfig c) {
        ...
        OptionGroupBuilder tools = group(b, "reanimated.group.tools")
            .addOption(screenOpt(b, "studio", "reanimated.opt.studio",
                prev -> Minecraft.getInstance().gui.setScreen(new AnimationStudioScreen(prev))))
            .addOption(screenOpt(b, "profile_editor", "reanimated.opt.profile_editor",
                prev -> Minecraft.getInstance().gui.setScreen(new AnimProfileEditorScreen(prev))))
            .addOption(screenOpt(b, "own_screen", "reanimated.config.open",
                prev -> Minecraft.getInstance().gui.setScreen(new ReAnimatedConfigScreen(prev))));
    ...
    private static OptionPageBuilder page(ConfigBuilder b, String key) {
        return b.createOptionPage().setName(Component.translatable(key));
    }

    private static OptionGroupBuilder group(ConfigBuilder b, String key) {
        return b.createOptionGroup().setName(Component.translatable(key));
    }
    ...
    private static <E extends Enum<E>> OptionBuilder enumOpt(ConfigBuilder b, String id, String key,
        ...
        return b.createEnumOption(id(id), type)
            ...
            .setElementNameProvider(v -> Component.literal(display.apply(v)))
            .setName(Component.translatable(key))
            .setTooltip(tip(id))
            .setDefaultValue(def)
    ...
    private static Component tip(String id) {
        return Component.translatable("reanimated.tip." + id);
    }

    private static Identifier id(String path) {
        return Identifier.fromNamespaceAndPath("reanimated", path);
    }
    ...
    private static final ControlValueFormatter PLAIN = v -> Component.literal(Integer.toString(v));
    private static final ControlValueFormatter PIXELS = v -> Component.literal(v + " px");
    private static final ControlValueFormatter MILLIS = v -> Component.literal(v + " ms");
    private static final ControlValueFormatter PERCENT = v -> Component.literal(v + "%");
    private static final ControlValueFormatter TICKS = v -> Component.literal(v + " t");
}
